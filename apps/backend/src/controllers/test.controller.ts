...
export async function getTest(req: Request, res: Response) {
  try {
    const item = await prisma.test.findFirst({
      where: { slug: Array.isArray(req.params.slug) ? req.params.slug[0] : req.params.slug, isActive: true },
      include: { category: true },
    });
...
export async function submitTest(req: Request, res: Response) {
  try {
    const test = await prisma.test.findFirst({ where: { slug: Array.isArray(req.params.slug) ? req.params.slug[0] : req.params.slug, isActive: true } });
